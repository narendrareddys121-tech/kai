...
type ViewType = 'dashboard' | 'analyzer' | 'compare' | 'bookmarks' | 'profile';
    ...
    <div className={`min-h-screen flex flex-col ${resolvedTheme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <header className={`sticky top-0 z-50 backdrop-blur-md border-b ${resolvedTheme === 'dark' ? 'bg-slate-800/80 border-slate-700' : 'bg-white/80 border-slate-200'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-6">
            ...
            >
              ...
              <h1 className={`text-xl font-bold tracking-tight ${resolvedTheme === 'dark' ? 'text-slate-100' : 'text-slate-800'}`}>
                kai <span className="text-indigo-600 font-normal text-sm">v2.0</span>
              </h1>
            </button>
          ...
          <div className="flex items-center space-x-4">
            <div className={`hidden sm:flex items-center space-x-2 text-sm ${resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
              <span className="flex items-center"><span className="w-2 h-2 bg-emerald-500 rounded-full mr-2 animate-pulse"></span>Online</span>
              {user?.subscription === 'pro' && (
                <span className="px-2 py-1 bg-gradient-to-r from-yellow-400 to-orange-400 text-slate-900 rounded-full text-xs font-bold">
                  PRO
                </span>
              )}
            </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        {currentView === 'dashboard' && (
          <Dashboard onNavigate={setCurrentView} currentView={currentView} />
        )}
        
        {currentView === 'compare' && (
          <ComparisonTool theme={resolvedTheme} />
        )}
        
        {currentView === 'profile' && (
          <div className={`rounded-2xl p-8 ${resolvedTheme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
            <h2 className="text-2xl font-bold mb-6">Profile Settings</h2>
            <div className="space-y-6">
              <div className="flex items-center space-x-4">
                {user?.avatar && <img src={user.avatar} alt={user.name} className="w-20 h-20 rounded-full" />}
                <div>
                  <h3 className="text-xl font-bold">{user?.name}</h3>
                  <p className={resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-600'}>{user?.email}</p>
                  {user?.subscription && (
        ...
        {currentView === 'bookmarks' && (
          <div className={`rounded-2xl p-8 ${resolvedTheme === 'dark' ? 'bg-slate-800 border border-slate-700' : 'bg-white border border-slate-200'}`}>
            ...
            <p className={resolvedTheme === 'dark' ? 'text-slate-400' : 'text-slate-600'}>
              Your bookmarked analyses will appear here. Start analyzing products to save your favorites!
            </p>
          </div>
        )}
      ...
      {currentView === 'analyzer' && state.result && (
      ...
      {/* History Sidebar */}
      {currentView === 'analyzer' && showHistory && (
      ...
      {/* Camera Modal */}
      {currentView === 'analyzer' && showCamera && (
